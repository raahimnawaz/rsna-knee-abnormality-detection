...
| **4c-4** | **⛔ THE THREE-ARM BLEND SCORED **0.914** ON LB — REAL, AND HALF OF WHAT WAS PREDICTED. `2026-08-18`** Submission `55608011`, banked two-arm **0.908** → three-arm **0.914**, so the public RadImageNet arm **earned its slot: +0.006**. But §4b's debias+offset chain predicted **0.921**, i.e. **+0.013 — the delivered gain is 46% of it.** ⚠️ **This is §3v's sub-additivity again and it is now measured on the LB rather than argued**: gold **+0.0135** and large-n **+0.0102 at 8.8σ** both translated to **+0.006** shipped. The arm is not refuted — the sign is right, the CI [+0.0004, +0.0262] contains it, and 8.8σ was never a claim about magnitude. **What is refuted is the gold→LB MAGNITUDE transfer.** §3l-2's offset predicts *level* far better than it predicts *increment*. ⛔ **Price every future arm's LB gain at roughly HALF its gold gain until this is re-measured** — §4b's +0.0135 arm bought +0.006, and any plan that sums gold increments is now known to over-count by ~2x |
