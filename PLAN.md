...
|---|---|---|---|
| **F6** | **Blend the free diverse arms** — DINOv2 ViT-B/14@336, DINOv3 ViT-S/16, RadImageNet R50 | **weights downloaded**; local scoring + **1 submission** | **DO THIS FIRST (§3l-1, §9e).** The strong-AND-diverse member §2y proved we cannot build: our port had rank corr 0.639 and lacked only strength. Reproduce one family locally, score every family on `score_gold.py`, pre-register the blend, *then* spend the submission — batched with F1. |
